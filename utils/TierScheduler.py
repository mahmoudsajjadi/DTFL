...
def TierScheduler(computation_time_clients, T_max, **kwargs):
    ...
    if kwargs:
        ...
        client_epoch = kwargs['client_epoch']
        num_users = kwargs['num_users']
        dataset_size = kwargs['dataset_size']
        batch_size = kwargs['batch_size']
        data_transmitted_client = kwargs['data_transmitted_client_all']
    ...
    #### profiling
    MB = 1024 ** 2
    # total_data_size_tier = {1:314.5 * MB, 2:313.9 * MB, 3:625.6 * MB, 4:625.2 * MB, 5:1250.1 * MB, 6:1250.3 * MB, 7:312.6 * MB}
    total_data_size_tier = {1:0.629 * MB, 2:313.9 * MB, 3:625.6 * MB, 4:625.2 * MB, 5:1250.1 * MB, 6:1250.3 * MB, 7:312.6 * MB}
    # profile_client_side = {1: 0.019, 2: 0.037, 3: 0.060, 4: 0.065, 5: 0.118, 6: 0.160, 7: 0.0}
    # profile_server_side = {1: 0.133, 2: 0.105, 3: 0.098, 4: 0.063, 5: 0.026, 6: 0.005, 7: 0.0} 
    ...
    for k in range(num_users):
        ...
        print([f'{time:.2f}' for time in time_estimation.values()])
        # time_estimation_client[k] = min(time_estimation)
